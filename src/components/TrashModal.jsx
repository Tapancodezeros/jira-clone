...
export default function TrashModal({ projectId, onClose, onUpdate }) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-md p-4 animate-in fade-in duration-300">
            <div className="glass-panel rounded-2xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[85vh] overflow-hidden border border-slate-100 dark:border-slate-800">
                {/* Header */}
                <div className="p-5 border-b border-slate-200/50 dark:border-slate-700/50 flex items-center justify-between bg-white/50 dark:bg-slate-800/50 backdrop-blur-sm">
                    <div className="flex items-center gap-3">
                        <div className="p-2.5 bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 rounded-xl">
                            <Trash2 size={24} />
                        </div>
                        <div>
                            <h2 className="text-xl font-bold text-slate-800 dark:text-white">Trash Bin</h2>
                            <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">{deletedTasks.length} items found</p>
                        </div>
                    </div>
                ...
                {/* Toolbar */}
                <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex gap-3 items-center bg-white/30 dark:bg-slate-900/30 sticky top-0 z-10 backdrop-blur-md">
                    <div className="relative flex-1">
                        ...
                        <input
                            ...
                            className="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-slate-800 border-none rounded-xl text-sm focus:ring-2 focus:ring-blue-500/20 focus:bg-white dark:focus:bg-slate-900 transition-all outline-none text-slate-800 dark:text-slate-200"
                        />
                    </div>
                    {selectedIds.size > 0 && (
                ...
                <div className="flex-1 overflow-y-auto p-4 custom-scrollbar bg-slate-50/30 dark:bg-slate-900/30">
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center h-64 text-slate-400">
                            <div className="w-20 h-20 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center mb-4 inner-shadow">
                                <Trash2 size={40} className="text-slate-300 dark:text-slate-600" />
                            </div>
                            <p className="text-sm font-medium text-slate-500 dark:text-slate-400">No deleted items found</p>
                            {searchQuery && <p className="text-xs mt-1">Try clearing your search query</p>}
                        </div>
                    ) : (
                        <div className="space-y-2">
                            <div className="px-4 py-2 flex items-center gap-3 text-xs font-bold text-slate-400 uppercase tracking-wider">
                                ...
                                return (
                                    <div
                                        ...
                                        className={`group flex items-center gap-4 p-4 rounded-xl border transition-all cursor-pointer ${isSelected
                                            ...
                                            : 'bg-white dark:bg-slate-800 border-slate-100 dark:border-slate-700 hover:border-slate-200 dark:hover:border-slate-600 hover:shadow-md hover:-translate-y-0.5'
                                            }`}
                                        onClick={() => handleSelect(task.id)}
                                    >
                                        <button
                                            onClick={(e) => { e.stopPropagation(); handleSelect(task.id); }}
                                            className={`transition-colors ${isSelected ? 'text-blue-600' : 'text-slate-300 dark:text-slate-600 group-hover:text-slate-400'}`}
                                        >
                                            {isSelected ? <CheckSquare size={20} /> : <Square size={20} />}
                                        </button>

                                        <div className="flex-1 min-w-0">
                                            <h4 className={`text-sm font-bold truncate ${isSelected ? 'text-blue-900 dark:text-blue-100' : 'text-slate-800 dark:text-slate-200'}`}>
                                                {task.title}
                                            </h4>
                                            <p className="text-xs text-slate-500 dark:text-slate-400 truncate mt-1 max-w-[90%]">
                                                {task.description || 'No description provided'}
                                            </p>
                                        </div>

                                        <div className="flex items-center gap-4">
                                            <div className="text-xs text-right text-slate-400 font-medium hidden sm:block">
                ...
                {/* Footer */}
                <div className="p-4 bg-slate-50/80 dark:bg-slate-800/80 border-t border-slate-100 dark:border-slate-700 flex justify-between items-center text-xs text-slate-500 dark:text-slate-400 backdrop-blur-sm">
                    <div>
                        Selected: <span className="font-bold text-slate-700 dark:text-slate-200">{selectedIds.size}</span>
                    </div>
                    <div>
